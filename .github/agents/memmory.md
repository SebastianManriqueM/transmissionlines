...
- 2026-09-25: Sphinx sources live in `docs/source`; use `uv sync --locked --group docs`, then `uv run sphinx-build -W --keep-going -b html docs/source docs/build/html` and `... -b doctest docs/source docs/build/doctest`. Keep `docs/build`, autosummary output, `.coverage`, `__pycache__`, and `*.egg-info` ignored.